...
describe('Chapter 7: API Tests', () => {
  ...
  it('should return 200 with success message when security questions are correct', async () => {
    const res = await request(app)
  ...
  it('should return 400 with Bad Request when AJV validation fails', async () => {
    const res = await request(app)
      .post('/api/users/test@example.com/verify-security-question')
      .send({
        // Missing required fields → triggers AJV failure
      });
  ...
  it('should return 401 with Unauthorized when answers are incorrect', async () => {
    const res = await request(app)
      .post('/api/users/test@example.com/verify-security-question')
      .send({
        question1: 'Wrong',
